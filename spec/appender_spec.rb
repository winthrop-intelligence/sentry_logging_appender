...
RSpec.describe SentryLoggingAppender::Appender do
  ...
  let(:log_struct) do
    Class.new(Struct.new(
      ...
      def each_exception
        begin
          if exception && exception.respond_to?(:level_index) && exception.level_index && exception.level_index.zero?
            yield exception
          end
  ...
  before do
    ...
    allow(::Sentry).to receive(:logger).and_return(sentry_logger)
  ...
  it "skips logging if Sentry is not initialized" do
    ...
    expect(appender.log(log_event)).to eq(false)
  end
  ...
  it "logs exceptions as attributes" do
    ...
    def exception.backtrace; ["line 1", "line 2"]; end
    def exception.level_index; 0; end
